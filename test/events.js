$(function () {
  module("Createbone.Events");

  test("on and trigger", 2, function () {
    var obj = { counter: 0 };
    _.extend(obj, Createbone.Events);
  ...
  test("binding and trigger with event maps", function () {
    var obj = { counter: 0 };
    _.extend(obj, Createbone.Events);
  ...
  test("listenTo and stopListening", 1, function () {
    var a = _.extend({}, Createbone.Events)
      , b = _.extend({}, Createbone.Events);
  ...
  test("stopListening with omitted args", 2, function () {
    var a = _.extend({}, Createbone.Events);
    var b = _.extend({}, Createbone.Events);
  ...
  test("listenTo yourself", 1, function(){
    var e = _.extend({}, Createbone.Events);
  ...
  test("bind a callback with a supplied context", 1, function () {
    ...
    var obj = _.extend({},Createbone.Events);
  ...
  test("callback list is not altered during trigger", 2, function () {
    var counter = 0, obj = _.extend({}, Createbone.Events);
  ...
  test("once variant one", 3, function() {
    ...
    var a = _.extend({}, Createbone.Events).once('event', f);
    var b = _.extend({}, Createbone.Events).on('event', f);
  ...
  test("Createbone object inherits Events", function() {
    ok(Createbone.on === Createbone.Events.on);
  });

  asyncTest("once with asynchronous events", 1, function() {
    var func = _.debounce(function() { ok(true); start(); }, 50);
    var obj = _.extend({}, Createbone.Events).once('async', func);
  ...
  test("`once` on `all` should work as expected", 1, function() {
    Createbone.once('all', function() {
      ok(true);
      Createbone.trigger('all');
    });
    Createbone.trigger('all');
  });

  test("once without a callback is a noop", 0, function() {
    _.extend({}, Createbone.Events).once('event').trigger('event');
  });

  test("event functions are chainable", function() {
    var obj = _.extend({}, Createbone.Events);
    var obj2 = _.extend({}, Createbone.Events);
    var fn = function() {};
