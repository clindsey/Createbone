$(document).ready(function() {
  ...
  module("Createbone.Collection", _.extend(new Environment, {

    setup: function() {
      Environment.prototype.setup.apply(this, arguments);

      a         = new Createbone.Model({id: 3, label: 'a'});
      b         = new Createbone.Model({id: 2, label: 'b'});
      c         = new Createbone.Model({id: 1, label: 'c'});
      d         = new Createbone.Model({id: 0, label: 'd'});
      e         = null;
      col       = new Createbone.Collection([a,b,c,d]);
      otherCol  = new Createbone.Collection();
    }
  ...
  test("String comparator.", 1, function() {
    var collection = new Createbone.Collection([
      {id: 3},
      {id: 1},
      {id: 2}
    ], {comparator: 'id'});
  ...
  test("new and parse", 3, function() {
    var Collection = Createbone.Collection.extend({
      parse : function(data) {
  ...
  test("get with non-default ids", 5, function() {
    var col = new Createbone.Collection();
    var MongoModel = Createbone.Model.extend({idAttribute: '_id'});
    ...
    var col2 = new Createbone.Collection();
  ...
  test("add", 10, function() {
    ...
    e = new Createbone.Model({id: 10, label : 'e'});
    otherCol.add(e);
    ...
    var f = new Createbone.Model({id: 20, label : 'f'});
    var g = new Createbone.Model({id: 21, label : 'g'});
    var h = new Createbone.Model({id: 22, label : 'h'});
    var atCol = new Createbone.Collection([f, g, h]);
  ...
  test("add multiple models", 6, function() {
    var col = new Createbone.Collection([{at: 0}, {at: 1}, {at: 9}]);
  ...
  test("add; at should have preference over comparator", 1, function() {
    var Col = Createbone.Collection.extend({
    ...
    col.add(new Createbone.Model({id: 1}), {at:   1});

    equal(col.pluck('id').join(' '), '3 1 2');
  });

  test("can't add model to collection twice", function() {
    var col = new Createbone.Collection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
  ...
  test("add model to multiple collections", 10, function() {
    var counter = 0;
    var e = new Createbone.Model({id: 10, label : 'e'});
    ...
    var colE = new Createbone.Collection([]);
    colE.on('add', function(model, collection) {
      equal(e, model);
      equal(colE, collection);
    });
    var colF = new Createbone.Collection([]);
  ...
  test("add model with parse", 1, function() {
    ...
    var Col = Createbone.Collection.extend({model: Model});
    var col = new Col;
  ...
  test("add model to collection with sort()-style comparator", 3, function() {
    ...
    var tom = new Createbone.Model({name: 'Tom'});
    var rob = new Createbone.Model({name: 'Rob'});
    var tim = new Createbone.Model({name: 'Tim'});
    col.add(tom);
    col.add(rob);
    col.add(tim);
  ...
  test("shift and pop", 2, function() {
    var col = new Createbone.Collection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
  ...
  test("events are unbound on remove", 3, function() {
    var counter = 0;
    var dj = new Createbone.Model();
    var emcees = new Createbone.Collection([dj]);
  ...
  test("remove in multiple collections", 7, function() {
    ...
    var passed = false;
    var e = new Createbone.Model(modelData);
    var f = new Createbone.Model(modelData);
    f.on('remove', function() {
      passed = true;
    });
    var colE = new Createbone.Collection([e]);
    var colF = new Createbone.Collection([f]);
    ok(e != f);
    ok(colE.length === 1);
    ok(colF.length === 1);
    colE.remove(e);
  ...
  test("remove same model in multiple collection", 16, function() {
    var counter = 0;
    var e = new Createbone.Model({id: 5, title: 'Othello'});
    ...
    var colF = new Createbone.Collection([e]);
  ...
  test("Colllection: non-persisted model destroy removes from all collections", 3, function() {
    var e = new Createbone.Model({title: 'Othello'});
  ...
  */
  ...
  test("create with validate:true enforces validation", 2, function() {
    var ValidatingModel = Createbone.Model.extend({
      validate: function(attrs) {
        return "fail";
      }
    });
    var ValidatingCollection = Createbone.Collection.extend({
  ...
  test("where and findWhere", 8, function() {
    var model = new Createbone.Model({a: 1});
    var coll = new Createbone.Collection([
      model,
      {a: 1},
      {a: 1, b: 2},
      {a: 2, b: 2},
  ...
  test("sortedIndex", function () {
    var model = new Createbone.Model({key: 2});
    var collection = new (Createbone.Collection.extend({
      comparator: 'key'
    }))([model, {key: 1}]);
  ...
  test ("reset with different values", function(){
    var col = new Createbone.Collection({id: 1});
    col.reset({id: 1, a: 1});
    equal(col.get(1).get('a'), 1);
  });

  test("same references in reset", function() {
    var model = new Createbone.Model({id: 1});
    var collection = new Createbone.Collection({id: 1});
  ...
  test("reset passes caller options", 3, function() {
    ...
    var col = new (Createbone.Collection.extend({ model: Model }))();
  ...
  test("add does not alter arguments", 2, function(){
    var attrs = {};
    var models = [attrs];
    new Createbone.Collection().add(models);
  ...
  test("Invalid models are discarded with validate:true.", 5, function() {
    ...
    collection.model = Createbone.Model.extend({
  ...
  test("multiple copies of the same model", 3, function() {
    var col = new Createbone.Collection();
    var model = new Createbone.Model();
    col.add([model, model]);
    equal(col.length, 1);
    col.add([{id: 1}, {id: 1}]);
    equal(col.length, 2);
  ...
  test("#964 - collection.get return inconsistent", 2, function() {
    var c = new Createbone.Collection();
  ...
  test("#1112 - passing options.model sets collection.model", 2, function() {
    var Model = Createbone.Model.extend({});
    var c = new Createbone.Collection([{id: 1}], {model: Model});
    ok(c.model === Model);
  ...
  test("null and undefined are invalid ids.", 2, function() {
    var model = new Createbone.Model({id: 1});
    var collection = new Createbone.Collection([model]);
    model.set({id: null});
  ...
  test("#1355 - `options` is passed to success callbacks", 2, function(){
    var m = new Createbone.Model({x:1});
    var col = new Createbone.Collection();
    var opts = {
  ...
  test("#1412 - Trigger 'request' and 'sync' events.", 4, function() {
    var collection = new Createbone.Collection;
    collection.url = '/test';
    Createbone.ajax = function(settings){ settings.success(); };
  ...
  */
  ...
  test("#1655 - groupBy can be used with a string argument.", 3, function() {
    var collection = new Createbone.Collection([{x: 1}, {x: 2}]);
  ...
  test("#1655 - sortBy can be used with a string argument.", 1, function() {
    var collection = new Createbone.Collection([{x: 3}, {x: 1}, {x: 2}]);
  ...
  */
  ...
  test("`sort` shouldn't always fire on `add`", 1, function() {
    var c = new Createbone.Collection([{id: 1}, {id: 2}, {id: 3}], {
      comparator: 'id'
    });
  ...
  test("set", function() {
    var m1 = new Createbone.Model();
    var m2 = new Createbone.Model({id: 2});
    var m3 = new Createbone.Model();
    var c = new Createbone.Collection([m1, m2]);
  ...
  test("set + merge with default values defined", function() {
    ...
    var col = new Createbone.Collection([m], {model: Model});
  ...
  test('merge without mutation', function () {
    ...
    var Collection = Createbone.Collection.extend({model: Model});
  ...
  test('`set` matches input order in the absence of a comparator', function () {
    var one = new Createbone.Model({id: 1});
    var two = new Createbone.Model({id: 2});
    var three = new Createbone.Model({id: 3});
    var collection = new Createbone.Collection([one, two, three]);
  ...
  */
  ...
  test("`set` with non-normal id", function() {
    var Collection = Createbone.Collection.extend({
      model: Createbone.Model.extend({idAttribute: '_id'})
    });
  ...
  asyncTest("#1939 - `parse` is passed `options`", 1, function () {
    ...
    var ajax = Createbone.ajax;
    Createbone.ajax = function (params) {
